...
sikuregex = re.compile(r"""
    ...
    部     # Match 'category' (bu)
    [一二三四五六七八九十百零]+     # match numbers
    \n+-+\n+ # This is the junk between what we need

    # sometimes the title of the work appears here
    (?: # open group, '?:' means don't capture as we don't need to return this
    《四庫全書總目提要》紀昀等      # title of work
    )?  # close group, make optioal
    \n*? # match new lines 0 or more times

    # Subcategory information appears after either a == or ○
    ...
    (.+?)   # match the sub-class (and capture it)
    類      # match 'category' (lei)
    """, 
...
for page in pages:
    ...
    if siku_info:
        ...
        # the second capture group is the subclassification
        current_subcat = siku_info.group(2)
